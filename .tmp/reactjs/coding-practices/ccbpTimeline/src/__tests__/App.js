...
describe(':::RJSCPRXL8I_TEST_SUITE_1:::CCBP Timeline tests', () => {
  ...
  it(':::RJSCPRXL8I_TEST_16:::Page should consist of HTML main heading elements inside the timeline cards with font-family value as "Roboto":::5:::', () => {
    render(<App />)
    expect(
      screen.getByRole('heading', {
        name: timelineItemsList[0].courseTitle,
  ...
  it(':::RJSCPRXL8I_TEST_17:::Page should consist of HTML anchor elements with text content as "Visit" and the color as "#0967d2":::5:::', () => {
    render(<App />)
    expect(screen.getAllByText(/Visit/i, {exact: false})[0]).toHaveStyleRule(
      'color',
      expect.stringContaining('#0967d2'),
    )
  })
})

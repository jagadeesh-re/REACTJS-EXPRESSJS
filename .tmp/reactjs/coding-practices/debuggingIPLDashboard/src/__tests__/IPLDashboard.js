...
describe(':::RJSCPNE8JJ_TEST_SUITE_1:::Home Route tests', () => {
  beforeAll(() => {
    server.listen()
  })
  afterEach(() => {
  ...
  it(':::RJSCPNE8JJ_TEST_1:::Page should consist of at least two HTML list items and the teams list received in the response should be rendered using a unique key as a prop for each team card item:::5:::', async () => {
  ...
  it(':::RJSCPNE8JJ_TEST_2:::Home Route should consist of an HTML main heading element with text content as "IPL Dashboard":::5:::', async () => {
    renderWithBrowserRouter()
    expect(
      await screen.findByText(teamsListResponse.teams[0].name),
    ).toBeInTheDocument()
  ...
  it(':::RJSCPNE8JJ_TEST_3:::Home Route should consist of an HTML image element with alt attribute value as "ipl logo" and src attribute value as the URL for IPL Logo image:::5:::', async () => {
    renderWithBrowserRouter()
    expect(
      await screen.findByText(teamsListResponse.teams[0].name),
    ).toBeInTheDocument()
  ...
  it(':::RJSCPNE8JJ_TEST_4:::When the Home Route is opened, it should initially consist of an HTML container element with testid attribute value as "loader":::5:::', async () => {
    renderWithBrowserRouter()
    await waitForElementToBeRemoved(() => screen.queryByTestId('loader'))
  })

  it(':::RJSCPNE8JJ_TEST_5:::When the Home Route is opened, an HTTP GET request should be made to the given teamsApiUrl to get the list of IPL teams:::5:::', async () => {
  ...
  it(':::RJSCPNE8JJ_TEST_6:::When the HTTP GET request made in Home Route is successful, then the page should consist of HTML paragraph elements with text content equal to the "name" in teams received in the response:::5:::', async () => {
    renderWithBrowserRouter()
    expect(
      await screen.findByText(teamsListResponse.teams[0].name),
    ).toBeInTheDocument()
  ...
  it(':::RJSCPNE8JJ_TEST_7:::When the HTTP GET request made in Home Route is successful, then the page should consist of HTML image elements with alt and src values equal to the "name" and "team_image_url" in teams received in the response:::5:::', async () => {
    renderWithBrowserRouter()
    expect(
      await screen.findByText(teamsListResponse.teams[0].name),
    ).toBeInTheDocument()
  ...
  it(':::RJSCPNE8JJ_TEST_8:::When a team card is clicked in Home Route, then the page should be navigated to the Team Matches Route with "/team-matches/:id" in the URL:::5:::', async () => {
  ...
  it(':::RJSCPNE8JJ_TEST_9:::When a "/bad-path" is provided in the URL, then the page should be navigated to Not Found Route and consist of an HTML heading element with text content as "Page Not Found":::5:::', async () => {
  ...
  it(':::RJSCPNE8JJ_TEST_10:::Page should consist of at least eight team cards wrapped with the Link from react-router-dom each inside a HTML list item:::5:::', async () => {
    const {container} = renderWithBrowserRouter()
    expect(
      await screen.findByText(teamsListResponse.teams[1].name),
    ).toBeInTheDocument()

    const listElements = container.querySelectorAll('li > a')

    expect(listElements.length).toBeGreaterThanOrEqual(8)
  })
})

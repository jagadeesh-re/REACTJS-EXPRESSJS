...
import ImagesList from '../imageslist/index'
...
class MatchGame extends Component {
  state = {
    ...
    score: 0,
    seconds: 60,
    randomurl: imagesList[0].imageUrl,
  }

  displayTab = id => this.setState({active: id})

  matchfun = imageurl => {
    const {randomurl} = this.state
    if (imageurl === randomurl) {
      const newRandomId = Math.ceil(Math.random() * 30)
      const newRandomUrl = imagesList[newRandomId].imageUrl
      this.setState(prevState => ({
        randomurl: newRandomUrl,
  ...
  clear = () => clearInterval(this.testId)

  render() {
    const {active, list, score, seconds, randomurl} = this.state
    const {tabsList} = this.props

    if (seconds === 0) this.clear()
    ...
    return (
      <div className="container">
        ...
        <div className="inner-container">
          {seconds !== 0 ? (
            <div>
              <img src={randomurl} className="imageUrl-container" alt="match" />

              <ul className="tablist-headers">
              ...
              <ul className="unordered-list">
                {list.map(eachItem => {
                  if (active.toUpperCase() === eachItem.category.toUpperCase())
                    return (
                      <ImagesList
                        eachItem={eachItem}
                        key={eachItem.id}
                        matchfun={this.matchfun}
                      />
                    )
                  return ''
                })}
            ...
            <div className="score-card-container">
              ...
              <button type="button" className="play-again" onClick={this.reset}>
                <img

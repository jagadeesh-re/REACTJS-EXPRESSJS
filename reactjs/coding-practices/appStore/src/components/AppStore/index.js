...
class AppStore extends Component {
  state = {list: appsList, tabId: tabsList[0].tabId}

  changeColour = unique => {
    const tabId = tabsList.filter(each => each.tabId === unique)
    this.setState({tabId: tabId[0].tabId})
  }

  change = event => {
    const newlist = appsList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState(prevState => ({list: newlist, tabId: prevState.tabId}))
  }

  render() {
    const {list, tabId} = this.state
    const selectedlist = list.filter(each => each.category === tabId)
      ...
      <div className="container">
        ...
        <ul className="listItems">
          {selectedlist.map(eachItem => (
            <AppItem eachItem={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
